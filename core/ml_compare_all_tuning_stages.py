...
# =========================================================
# ROBUST COMPARISON
# BASELINE vs TUNING 1-5
# =========================================================

# Exact configurations of the complete tuning sequence.
# All models are evaluated on the SAME five independent
# validation sets (500 samples each), using seeds 2026-2030.
...
def compare_all_tuning_stages(
    X_train,
    y_train
):
    """
    Compare Baseline + Tuning 1-5 on exactly the same
    five independent validation sets.

    Each model is trained on the complete 1000-sample
    training dataset. No independent validation point is
    used during model fitting or tuning.

    The five validation sets use seeds 2026-2030 and contain
    500 samples each.
    """

    print("\n========================================")
    print("ROBUST COMPARISON")
    print("BASELINE vs TUNING 1-5")
    print("========================================")

    models_params = {
        "Baseline": BASELINE_XGB_PARAMS,
        "Tuning 1": TUNING_1_PARAMS,
        "Tuning 2": TUNING_2_PARAMS,
        "Tuning 3": TUNING_3_PARAMS,
        "Tuning 4": TUNING_4_PARAMS,
    ...
    # -----------------------------------------------------
    # Generate common validation sets ONCE.
    # Every model therefore sees exactly the same points.
    # -----------------------------------------------------
    ...
    # -----------------------------------------------------
    # Evaluate every model on every common validation set.
    # -----------------------------------------------------
    ...
    for model_name, params in models_params.items():
        ...
        for seed in ROBUSTNESS_SEEDS:
            ...
            X_validation, y_physical = validation_sets[seed]
    ...
    # =====================================================
    # SUMMARY: MEAN ± STD ACROSS THE FIVE SETS
    # =====================================================
    ...
    summary_df = (
        ...
        .agg(
            ...
            Top10_R2_mean=("Top10_R2", "mean"),
            Top10_R2_std=("Top10_R2", "std"),

            Top10_RMSE_mean=("Top10_RMSE", "mean"),
            Top10_RMSE_std=("Top10_RMSE", "std"),

            Top10_MAE_mean=("Top10_MAE", "mean"),
            Top10_MAE_std=("Top10_MAE", "std"),

            MuHigh_R2_mean=("MuHigh_R2", "mean"),
            MuHigh_R2_std=("MuHigh_R2", "std"),

            MuHigh_RMSE_mean=("MuHigh_RMSE", "mean"),
            MuHigh_RMSE_std=("MuHigh_RMSE", "std"),

            MuHigh_MAE_mean=("MuHigh_MAE", "mean"),
            MuHigh_MAE_std=("MuHigh_MAE", "std")
        )
        .reset_index()
    )

    # Keep the logical tuning order instead of sorting by RMSE.
    order = {
        "Baseline": 0,
        "Tuning 1": 1,
        "Tuning 2": 2,
    ...
    print("\n========================================")
    print(
        "FINAL RESULTS — MEAN ± STD OVER "
        "5 × 500 INDEPENDENT SAMPLES"
    )
    print("========================================")
    ...
    detailed_file = (
        OUTPUT_DIR /
        "ml_all_tuning_stages_5x500_detailed.csv"
    )

    summary_file = (
        OUTPUT_DIR /
        "ml_all_tuning_stages_5x500_summary.csv"
    )

    params_file = (
        OUTPUT_DIR /
        "ml_all_tuning_stages_parameters.csv"
    )
...
def main_compare_all():
    ...
    return compare_all_tuning_stages(
        X,
        y
    )
